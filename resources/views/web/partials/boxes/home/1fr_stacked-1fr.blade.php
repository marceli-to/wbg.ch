          ...
          @if ($elements[0]->projectimage)
            ...
            @else
              <a href="/projekt/{!! AppHelper::slug($elements[0]->projectimage->project) !!}" 
                ...
                title="{{$elements[0]->projectimage->project->name}}">
            @endif
              <img class="lazyload" data-src="{!! ImageHelper::get($elements[0]->projectimage->name, 'lg') !!}" height="430" width="280" alt="{{$elements[0]->projectimage->caption}}">
            </a>
          @endif
        @endif
      </div>
    ...
    <div>
      @if (isset($elements[2]))
        ...
        @if ($elements[2]->projectimage)
          @if ($elements[2]->projectimage->project->category_id == 3)
            <a href="/projekte/{{ $elements[2]->projectimage->project->category_id }}/panoptikum/{{ $elements[2]->projectimage->project->subcategory_id }}/-/!#{{str_slug($elements[2]->projectimage->project->name)}}" 
              rel="canonical" 
              title="{{$elements[2]->projectimage->project->name}}">
          @else
            <a href="/projekt/{!! AppHelper::slug($elements[2]->projectimage->project) !!}" 
              rel="canonical" 
              title="{{$elements[2]->projectimage->project->name}}">
          @endif
            <img class="lazyload" data-src="{!! ImageHelper::get($elements[2]->projectimage->name, 'lg') !!}" height="560" width="430" alt="{{$elements[2]->projectimage->caption}}">
          </a>
        @endif
      @endif
    </div>
